...
def _schema_prompt(schema_config: dict) -> str:
    return (
        ...
        f'Schema:\n{json.dumps(schema_config, ensure_ascii=False, indent=2)}\n\n'
    )
...
async def structure(
    ...
    if file_type == FileType.IMAGE:
        ...
        return await _structure_image(file_path, schema_config, model)
    model = str(params.get('model', settings.openrouter_llm_model))
    return await _structure_text(file_path, schema_config, model)


async def _structure_text(file_path: str, schema_config: dict, model: str) -> dict:
